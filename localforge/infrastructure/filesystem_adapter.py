"""
Filesystem adapter — file read/write and directory operations using pathlib.
...
class FileSystemAdapter:
    """Adapter providing file system operations via pathlib."""
    ...
    def _walk(self, base, current, ext_set, skip_dirs, result):
        try:
            entries = sorted(current.iterdir(), key=lambda p: (p.is_file(), p.name))
        except PermissionError:
            logger.warning("Permission error: %s", current)
            return
    ...
    def _iter_code_files_limit(self, root: Path, limit: int):
        count = 0
