"""
Ollama HTTP API wrapper with SSE streaming support.
The sole class implementing the LLMPort interface.
"""
...
def _detect_cuda() -> bool:
    """Return True if an NVIDIA CUDA-capable GPU is detected via nvidia-smi."""
    try:
...
class OllamaClient:
    """HTTP client for the Ollama API with streaming and model listing support."""
    ...
    def __init__(self, base_url: str = _DEFAULT_BASE_URL) -> None:
        ...
        if self.cuda_available:
            logger.info("CUDA GPU detected: setting num_gpu=-1 for Ollama requests")
        else:
    ...
    def set_num_thread(self, num_thread: Optional[int]) -> None:
        """Set the number of CPU threads Ollama should use (None = default auto)."""
    ...
    def list_models(self) -> List[str]:
        try:
            ...
            raise OllamaConnectionError(f"Failed to fetch model list: {exc}") from exc
    ...
    def stream_completion(
        ...
        """Stream text tokens from Ollama generate API."""
    ...
    def generate_sync(self, model: str, prompt: str, system: Optional[str] = None) -> str:
        """Non-streaming full-response generation (for tests and internal use)."""
